...
contract Oyla {
    ...
    modifier isOrganizerOfElec(uint _elecID) {
        require(msg.sender == elections[_elecID].organizer, "Auth error: Only organizer can do that");
        _;
    }
    
    modifier checkElectionCount() {
        require(electionCount <= MAX_ELECTION_COUNT, "You have reached the maximum election count");
        _;
    }
    ...
    modifier isElecNotExists(uint _elecID) {
        require(elections[_elecID].initialized == false, "Election already initialized");
        _;
    }
    ...
    function createElection(uint _elecID, string memory _name, uint _candidateCount, uint _voterCount) 
        public checkElectionCount isElecNotExists(_elecID) 
    {
        address organizer = msg.sender;
        elections[electionCount] = Election(electionCount, _name, _candidateCount, _voterCount, 0, organizer, true);
        electionCount++;
    }
